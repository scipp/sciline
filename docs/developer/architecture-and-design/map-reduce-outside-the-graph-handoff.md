...
- `Accumulator`, the structural protocol for what sits between stages: `push(value)`, `value`. `Buffered(func)` is a factory for an accumulator that holds all pushed values and applies an n-ary function on `value`. Both in sciline. ess.reduce's accumulators satisfy the protocol once `maybe_hist` leaves their base class; `clear` stays an ess.reduce convention. `Forwarder` (held context) stays in ess.reduce. Held contributions are a dict by member label in the package object.
- `Aggregation(pipeline, members, accumulators, outputs=())`: contribute stage, optional finalize stage, an accumulator factory per accumulation key, fresh accumulators per `combine`/`compute`; holds nothing; `contribute`/`combine`/`finalize`, `compute(table)` pushes each contribution as it is made. Table type `Mapping[label, Mapping[Key, value]]` only. Sciline, no experimental label (decided 2026-09-11; mechanism, not policy).
- Package objects own topology, state, and policy: pipeline, aggregations, shared finalize stage, contributions, and the rule for what a parameter change keeps (`key in agg.contribute_stage.keys`). `StreamProcessor` is the same kind of object for streams.
...
1. `Stage` + `Fold` (now `Aggregation`) with `as_pipeline()`, a bridge synthesizing providers so `with_sample_runs` could keep returning a pipeline. Rejected: the bridge was the only way aggregations composed, fixed the member table, hid a loop in a provider.
...
3. Stateless `Fold`, state in package objects. Current, under the name `Aggregation` with an accumulator per accumulation key.
...
- **One package object or many.** Decides whether the GUI protocol (set parameter, set members for a member key, compute) is a base class, a protocol, or one generic object from a registry of member key to accumulation keys. Recommendation: write esssans's by hand, then essreflectometry's, then decide.
...
- **Static work across processes.** Stages in one process share static work through `warm`; a contribute in a throwaway process recomputes it. The phase 3 cost the essapps stateless note measures.

Settled 2026-09-11:

- **Combine protocol.** An accumulator per accumulation key: sciline's `Accumulator` protocol, `push` and `value`, with `Buffered(func)` for n-ary functions; every existing ESS combine migrates through `Buffered`. An n-ary function forces buffering inside the aggregation; with accumulators the buffer-versus-incremental choice sits on the object the author passes, which matters for sum-like combines with large dense outputs.
- **Member tables.** `Mapping[label, Mapping[Key, value]]` only; pandas stays out.
- **Parallel members.** The caller's job; `Aggregation` takes no executor. `contribute` is a plain call to map over rows with threads, processes, or dask delayed, pushing results as they arrive. The user guide shows an example. (Lost the free dask parallelism of one graph, about 1.7 s of 7 s on LoKI.)
- **Names.** `Fold` becomes `Aggregation`; "cut key" becomes "accumulation key" (essapps already says "accumulation point"); `at=` becomes `accumulators=`; `fold.cut` becomes `agg.accumulation_keys`. `Stage`, `warm`, members, contribution, `contribute`/`combine`/`finalize`, `contribute_stage`/`finalize_stage`, `compute_members` unchanged. `Forwarder` stays in ess.reduce; its name is ess.reduce's call. Reasons: `fold` means reshaping in scipp; the contribute/combine/finalize shape is what Spark, Flink, Beam, and pandas call aggregation, and there the stateful object is the accumulator.
- **Experimental label.** None. The staged rollout (additive minor release, esssans migrates, breaking release) is the trial period.
...
1. **sciline, additive.** `Stage`, `warm`, `provide(key, callable)`, `Accumulator`, `Buffered`, `Aggregation`, `compute_members`; building a task graph with missing values (already what `HandleAsComputeTimeException` does). Replace the prototype's private access (`TaskGraph._graph`, `Provider.parameter`, `ArgSpec`) with public construction. Docs: a guide on stages and aggregations, with the parallel-members example over `contribute`. No breaking change.
2. **ess.reduce, additive.** `Forwarder`; move the `maybe_hist` default out of the accumulator base class's `push` so the accumulators satisfy sciline's `Accumulator` protocol as they are; rewrite `StreamProcessor` on `Stage` against its 35 tests (`streaming.py`, 1081 lines; `_FedWorkflow`, `_build_streaming_workflow`, `_find_descendants`, `_find_parents`, `_map_context_to_cached_nodes`, `allow_bypass`, `visualize` classification). map/reduce still present, nothing else changes.
3. **Packages, one at a time, each its own PR.** Replace the `with_*` helpers by a package object; migrate notebooks and tests. Sites, from the survey in the design doc:
   - esssans: `_set_runs`/`with_sample_runs`/`with_background_runs` (4 reduced keys × 2 run types, `merge_contributions`); `with_pixel_mask_filenames` becomes a `PixelMaskFilenames` list parameter and two providers (`DetectorMasks` reads `DetectorIDs` from `EmptyDetector[SampleRun]`, so the mask fold is inside per-run work); `with_banks` becomes a loop; `parameter_mappers` registrations in `workflow.py` lines 144-150; `beam_center_from_center_of_mass` callers use `with_pixel_mask_filenames`.
   - essreflectometry: up to 7 reduced keys, concat and `_any_value`, `try/except` around reduces that becomes the static-accumulation-key rule.
   - essspectroscopy (bifrost): two reduced keys at different depths; the esslivedata `EmptyDetector` bank fold.
   - essnmx: panels and MTZ files, notebooks use `Aggregation` directly.
...
4. **esslivedata.** Bifrost bank fold becomes `Aggregation(...).compute(...)` set as a parameter before the `StreamProcessor` is built; consumer of the rewritten `StreamProcessor`. Pins sciline until migrated.
...
6. **essapps.** Apply the suggested edits listed at the end of `stages.md` (D8, D13, D14, D15, glossary); the D3/D6 spike's fake workflow with two accumulation points is an `Aggregation`.
...
- `Aggregation` builds the probe stage and the contribute stage separately (two graph walks); fine for a prototype, tidy in the real thing.
...
- The user-guide example of parallel members over `contribute`.
