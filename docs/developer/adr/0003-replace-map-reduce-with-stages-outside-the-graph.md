...
- The planned reduction-service architecture (essapps) needs the same partition three times: caching everything upstream of a cheap parameter, splitting a fold into contribute, combine, and finalize so that members can be processed in separate processes, and running two halves of one graph in two processes.
...
2. **`Fold` and `compute_members`, in sciline.**
   Member keys are the keys supplied per repetition, such as `Filename[SampleRun]`; one row of values for them is a member.
   A cut key is a key at which per-member values are combined; a contribution is the dict of values at the cut keys for one member.
   A fold is two stages of one flat pipeline, contribute (member keys to cut keys) and finalize (cut keys to outputs, optional), with an n-ary combine per cut key between them.
   It holds no contributions and no member table; its stages hold their frontiers.
   Parameters are set on the pipeline before the fold is built; a changed parameter means a new fold.
   `fold.cut` lists the cut keys that depend on the members; a requested cut key that does not is computed by finalize instead.
   `contribute`, `combine`, and `finalize` are separate entry points, so the three steps can run in different processes with the contribution serialized between them; `compute(table)` is the loop for a caller that holds nothing.
...
3. **Connectors, in ess.reduce.**
   Objects between stages that follow one convention, `push`, `value`, `clear`: the accumulators of `ess.reduce.streaming`, which combine members, and a `Forwarder`, which holds a context until it is replaced.
   No shared base class; the convention is documented where they live.
   `maybe_hist` moves from `Accumulator.push` into the subclasses that need it.
...
4. **Drivers and package objects own topology, state, and policy.**
   ...
   esssans's object holds the pipeline, one fold per run type, the finalize stage over both cuts, and the contributions by filename; on a parameter change it rebuilds a fold if `key in fold.contribute_stage.keys` and the finalize stage if that reads the key.
...
Open, to settle before `Fold` lands or to release it as experimental:

- Whether `Fold` accepts an accumulator per cut key instead of, or besides, an n-ary function.
- Parallelism over members: an executor argument on `compute`, or the caller's concern.
- Names: stage, cut, contribution, contribute/combine/finalize, against essapps's "accumulation point" and "stage output".
...
- **Keep map/reduce, deprecated, and add `Stage` and `Fold` beside it.**
  ...
  The rollout is staged instead: stages and folds first, packages migrated one at a time, removal last.
...
- Sciline keeps a documented replacement: one `Fold` where the guide had `map(...).reduce(...)`.
  The parameter-tables guide becomes a guide on stages and folds.
...
- A requested cut key that does not depend on the members is computed by finalize without error; `fold.cut` reports it and package tests must check it.
...
- Each fold walks the graph at construction and again on every parameter change that reaches it.
